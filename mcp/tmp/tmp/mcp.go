...
func (debuggers) HyperDbgTestCommandParserShowTokens(command string, )  {
	 request[void]("HyperDbgTestCommandParserShowTokens", map[string]string{"addr": fmt.Sprintf("0x%x", address)})
}
func (debuggers) HyperDbgShowSignature()  {
	 request[void]("HyperDbgShowSignature", map[string]string{"addr": fmt.Sprintf("0x%x", address)})
}
...
func (debuggers) HyperDbgShowMemoryOrDisassemble(style , address uint64, memory_type , reading_type , pid uint32, size uint32, dt_details , )  {
	 request[void]("HyperDbgShowMemoryOrDisassemble", map[string]string{"addr": fmt.Sprintf("0x%x", address)})
}
